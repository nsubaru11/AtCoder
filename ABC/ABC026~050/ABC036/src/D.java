...
public final class D {
	...
	private static void solve() {
		...
		tree.addAll(n - 1, sc::nextInt0, sc::nextInt0);
		int[] deg = new int[n];
		setAll(deg, tree::degree);
		int[] q = new int[n];
		int t = 0;
		for (int i = 1; i < n; i++) {
			if (deg[i] != 1) continue;
		...
		fill(dp[1], 1);
		for (int h = 0; h < t; h++) {
			int u = q[h], v = tree.parent(u);
			dp[0][v] = (int) ((long) dp[0][v] * (dp[0][u] + dp[1][u]) % MOD);
			dp[1][v] = (int) ((long) dp[1][v] * dp[0][u] % MOD);
			if (v != 0 && --deg[v] == 1) q[t++] = v;
		}
		out.println((dp[0][0] + dp[1][0]) % MOD);
	}
