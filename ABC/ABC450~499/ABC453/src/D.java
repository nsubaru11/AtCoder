...
public final class D {
	...
	private static void solve() {
		...
		int start = 0;
		for (int i = 0; i < hw; i++) {
			if ((s[i] = sc.nextChar()) == 'S') start = i;
		}
		int[] path = new int[hw4];
		int[] visitCnt = new int[hw4];
		ArrayDeque<Integer> dq = new ArrayDeque<>(hw);
		fill(visitCnt, -1);
		visitCnt[start] = visitCnt[start + hw] = visitCnt[start + hw * 2] = visitCnt[start + hw * 3] = 0;
		dq.add(start);
		int goal = -1;
		outer:
		while (!dq.isEmpty()) {
			int u = dq.poll();
			int du = u / hw, cnt = visitCnt[u];
			int ij = u % hw, i = ij / w, j = ij % w;
			char su = s[ij];
			if (cnt == 5_000_000) continue;
			int from = su == 'o' ? du : 0, to = su == 'o' ? du + 1 : 4;
			for (int dv = from; dv < to; dv++) {
				if (su == 'x' && dv == du) continue;
				int ni = i + di[dv], nj = j + dj[dv];
				if (!isValidRange(ni, nj, h, w)) continue;
				int nij = ni * w + nj, v = nij + dv * hw;
				char sv = s[nij];
				if (sv == '#' || visitCnt[v] != -1) continue;
				path[v] = u;
				visitCnt[v] = cnt + 1;
				if (sv == 'G') {
					goal = v;
		...
		out.println(goal != -1);
		if (goal == -1) return;
		int i = visitCnt[goal], p = goal;
		char[] ans = new char[i];
		while (p != start) {
			ans[--i] = op[p / hw];
			p = path[p];
		}
		out.printChars(ans).println();
	}
