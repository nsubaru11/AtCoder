...
public final class E {
	...
	private static void solve() {
		int n = sc.nextInt();
		int m = sc.nextInt();
		TreeMap<Integer, TreeMap<Integer, Integer>> lr = new TreeMap<>();
		TreeMap<Integer, TreeMap<Integer, Integer>> rl = new TreeMap<>();
		int[] minR = new int[n + 2];
		fill(minR, Integer.MAX_VALUE);
		while (m-- > 0) {
			int l = sc.nextInt();
			int r = sc.nextInt();
			lr.computeIfAbsent(l, _ -> new TreeMap<>()).merge(r, 1, Integer::sum);
			rl.computeIfAbsent(r, _ -> new TreeMap<>()).merge(l, 1, Integer::sum);
			chmin(minR, l, r);
		}
		for (int i = n; i > 0; i--) {
			chmin(minR, i - 1, minR[i]);
		}
		int q = sc.nextInt();
		while (q-- > 0) {
			int s = sc.nextInt();
			...
			var startS = lr.get(s);
			var endT = rl.get(t);
			if (startS == null || endT == null) {
				out.println(false);
				continue;
			}
			var maxR = startS.floorEntry(t);
			var minL = endT.ceilingEntry(s);
			if (maxR == null || minL == null) {
				out.println(false);
				continue;
			}
			if (maxR.getKey() == t && minL.getKey() == s) {
				out.println(minL.getValue() >= 2 || minR[s] < t || minR[s + 1] <= t);
			} else {
				out.println(maxR.getKey() >= minL.getKey() - 1);
			}
		}
	}
