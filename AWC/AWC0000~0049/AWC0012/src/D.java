...
public final class D {
	...
	private static void solve() {
		int n = sc.nextInt();
		int m = sc.nextInt();
		char[] grid = sc.nextChars(n * m);
		ArrayDeque<Integer> dq = new ArrayDeque<>();
		dq.add(0);
		int[] visitCnt = new int[n * m];
		fill(visitCnt, 1, n * m, -1);
		outer:
		while (!dq.isEmpty()) {
			int u = dq.poll();
			int i = u / m, j = u % m;
			for (int d = 0; d < 4; d++) {
				...
				if (!isValidRange(ni, nj, n, m)) continue;
				int v = ni * m + nj;
				if (visitCnt[v] != -1) continue;
				if (grid[v] == '#') {
					dq.addLast(v);
					visitCnt[v] = visitCnt[u] + 1;
				} else {
					dq.addFirst(v);
					visitCnt[v] = visitCnt[u];
				}
				if (v == n * m - 1) break outer;
			}
		}
		out.println(visitCnt[n * m - 1]);
	}
