...
public class E {
	...
	private static class Tree {
		...
		public Tree(int n, int[] node) {
			this.n = n;
			this.tree = new ArrayList<>(n);
			for (int i = 0; i < n; i++)
				this.tree.add(new ArrayList<>(n));
			this.node = node;
		}

		public void addEdge(int u, int v, int cost) {
			this.tree.get(u).add(new Edge(u, v, cost));
			this.tree.get(v).add(new Edge(v, u, cost));
		}

		public long solve(int from, int i) {
			long ans = 0;
