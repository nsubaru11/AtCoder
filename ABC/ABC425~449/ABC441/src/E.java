...
public final class E {
	...
	private static void solve() {
		...
		parallelPrefix(cnt, Integer::sum);
		long ans = 0;
		IntSegmentTree segmentTree = new IntSegmentTree(2 * n + 1, Integer::sum, 0);
		ans += (cnt[2 * n] - cnt[n]) - segmentTree.query(n + 1, 2 * n);
		segmentTree.set(n, 1);
		for (int i = 0; i < n; i++) {
			int x = cnt[2 * n] - cnt[ps[i]];
			int y = segmentTree.query(ps[i] + 1, 2 * n);
			ans += x - y;
			segmentTree.update(ps[i], v -> v + 1);
		}
		out.println(ans);
	}
