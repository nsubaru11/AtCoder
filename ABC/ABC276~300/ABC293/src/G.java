...
public final class G {
	...
	private static void solve() {
		...
		int q = sc.nextInt();
		int[] a = sc.nextInt0(n);
		int[][] lr = sc.nextIntMatInv0(q, 2);
		for (int i = 0; i < q; i++) lr[1][i]++;
		long[] query = new long[q];
		int[] freq = new int[max(a) + 1];
		long[] ans = {0};
		MoAlgorithm.run(n, lr, i -> ans[0] += freq[a[i]] * (freq[a[i]]++ - 1L) / 2, i -> ans[0] -= (freq[a[i]] - 1L) * (freq[a[i]]-- - 2) / 2, i -> query[i] = ans[0]);
		out.println(query);
	}
	...
	private static void debugln(final Object... args) {
		if (DEBUG) {
			...
			else System.err.println(stream(args).map(G::stringify).collect(Collectors.joining("\n", "\n", "")));
		}
	}
