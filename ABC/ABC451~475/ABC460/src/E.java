...
public final class E {
	...
	private static void solve() {
		...
		while (t-- > 0) {
			...
			int m = sc.nextInt();
			int digit = digit10(n);
			int[] mpow10 = new int[digit + 1];
			long[] pow10 = new long[digit + 1];
			mpow10[0] = 1;
			pow10[0] = 1;
			for (int i = 1; i <= digit; i++) {
				mpow10[i] = (int) (mpow10[i - 1] * 10L % m);
				pow10[i] = 10 * pow10[i - 1];
			}
			pow10[digit] = n + 1;
			int ans = 0;
			for (int d = 1; d <= digit; d++) {
				int py = (int) ((pow10[d] - pow10[d - 1]) % MOD);
				int m2 = m / gcdInt(mpow10[d] - 1, m);
				ans = (int) ((ans + n / m2 % MOD * py % MOD) % MOD);
			}
			out.println(ans);
		}
	}
