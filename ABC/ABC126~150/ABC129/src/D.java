...
public final class D {
	...
	private static void solve() {
		int h = sc.nextInt();
		int w = sc.nextInt();
		boolean[][] grid = sc.nextBooleanMat(h, w, '.');
		int[][] pl = new int[h][w];
		int[][] pr = new int[h][w];
		int[][] pu = new int[h][w];
		int[][] pd = new int[h][w];
		for (int i = 0; i < h; i++) {
			if (grid[i][0]) pl[i][0]++;
			if (grid[i][w - 1]) pr[i][w - 1]++;
			for (int j = 1; j < w; j++) {
				if (grid[i][j]) pl[i][j] = pl[i][j - 1] + 1;
				if (grid[i][w - j - 1]) pr[i][w - j - 1] = pr[i][w - j] + 1;
			}
		}
		for (int j = 0; j < w; j++) {
			if (grid[0][j]) pu[0][j]++;
			if (grid[h - 1][j]) pd[h - 1][j]++;
			for (int i = 1; i < h; i++) {
				if (grid[i][j]) pu[i][j] = pu[i - 1][j] + 1;
				if (grid[h - i - 1][j]) pd[h - i - 1][j] = pd[h - i][j] + 1;
			}
		}
		int ans = 0;
		for (int i = 0; i < h; i++) {
			for (int j = 0; j < w; j++) {
				...
				ans = max(ans, pl[i][j] + pr[i][j] + pu[i][j] + pd[i][j] - 3);
			}
		}
		out.println(ans);
	}
