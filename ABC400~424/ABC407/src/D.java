...
public class D {
	...
	private static void solve(final FastScanner sc, final FastPrinter out) {
		...
		int w = sc.nextInt();
		int hw = h * w;
		long[] a = sc.nextLong(hw);
		long[] ans = new long[1];
		boolean[] memo = new boolean[hw];
		BiFunction<BiFunction, Long[], Void> dfs = (self, ps) -> {
			int pos = (int) ps[0].longValue();
			long sum = ps[1];
			if (pos == hw) {
				ans[0] = max(ans[0], sum);
				return null;
			...
			ps[0]--;
			if (i + 1 < h && !memo[pos + w]) {
				memo[pos] = memo[pos + w] = true;
				ps[0]++;
				self.apply(self, ps);
				ps[0]--;
				memo[pos] = memo[pos + w] = false;
			}
			if (j + 1 < w && !memo[pos + 1]) {
				memo[pos] = memo[pos + 1] = true;
				ps[0]++;
				self.apply(self, ps);
				ps[0]--;
				memo[pos] = memo[pos + 1] = false;
			}
			return null;
		};
		Long[] b = new Long[2];
