...
public final class E {
	...
	private static void solve() {
		...
		int m = sc.nextInt();
		int k = sc.nextInt();
		IndexedPriorityQueue pq = new IndexedPriorityQueue(n, true);
		IndexedPriorityQueue pq2 = new IndexedPriorityQueue(n);
		long sum = 0;
		for (int i = 0; i < m; i++) {
			int a = sc.nextInt();
			sum += a;
		...
		for (int i = 1, j = m; i < n - m + 1; i++, j++) {
			...
			sum += a;
			if (pq.contains(j - m)) {
				sum -= (int) pq.getCost(j - m);
				pq.remove(j - m);
			} else if (pq2.contains(j - m)) pq2.remove(j - m);
			if (!pq2.isEmpty()) {
				int pi = pq2.peekNode(), pa = (int) pq2.poll();
				pq.push(pi, pa);
				sum += pa;
				while (pq.size() > k) {
					int i2 = pq.peekNode();
