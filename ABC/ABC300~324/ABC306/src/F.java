...
public final class F {
	...
	private static void solve() {
		int n = sc.nextInt();
		int m = sc.nextInt();
		int[][] a = sc.nextIntMat(n, m);
		compression(a, n, m);
		IntBIT bit = new IntBIT(n * m);
		long ans = 0;
		for (int i = n - 2; i >= 0; i--) {
			for (int aj : a[i + 1]) bit.apply(aj, 1);
			for (int aj : a[i]) ans += bit.query(aj);
		}
		out.println(ans + (m * (m + 1L) / 2 * n * (n - 1) / 2));
	}
	...
	private static final class IntBIT {
		private final int n;
