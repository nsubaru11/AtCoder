...
public final class G {
	...
	private static void solve() {
		...
		int b = max(1, n / iSqrt(q));
		int iMask = (1 << 20) - 1;
		for (int i = 0; i < q; i++) {
			long l = lr[0][i] = sc.nextInt0();
			long r = lr[1][i] = sc.nextInt0();
			long lb = l / b;
			if ((lb & 1) == 1) r = n - r;
			queries[i] = (lb << 40) | (r << 20) | i;
		}
		sort(queries);
		int[] hm = new int[max(a) + 1];
		long ans = 0;
		long[] query = new long[q];
		for (int i = 0, l = 0, r = 0; i < q; i++) {
			int idx = (int) (queries[i] & iMask);
			int lp = lr[0][idx], rp = lr[1][idx];
			while (r <= rp) {
				int x = hm[a[r++]]++;
				ans += x * (x - 1L) / 2;
			}
