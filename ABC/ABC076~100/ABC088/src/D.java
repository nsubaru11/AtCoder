...
public final class D {
	...
	private static void solve() {
		...
		int ans = 0;
		for (int i = 1; i < hw - 1; i++) if (grid[i] == '.') ans++;
		ArrayDeque<Integer> dq = new ArrayDeque<>();
		dq.add(0);
		int[] visiteCnt = new int[hw];
		fill(visiteCnt, -1);
		visiteCnt[0] = 0;
		while (!dq.isEmpty()) {
			int from = dq.poll();
			int cnt = visiteCnt[from];
			int i = from / w, j = from % w;
			for (int d = 0; d < 4; d++) {
				int ni = i + di[d], nj = j + dj[d];
				if (!isValidRange(ni, nj, h, w)) continue;
				int to = ni * w + nj;
				if (visiteCnt[to] != -1 || grid[to] == '#') continue;
				dq.add(to);
				visiteCnt[to] = cnt + 1;
			}
		}
		out.println(visiteCnt[hw - 1] == -1 ? -1 : ans - visiteCnt[hw - 1] + 1);
	}
