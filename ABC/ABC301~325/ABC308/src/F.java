...
public final class F {
	...
	private static void solve() {
		int n = sc.nextInt();
		int m = sc.nextInt();
		int[] p = sc.nextSortedInt(n);
		IntPriorityQueue pq = new IntPriorityQueue(m, true);
		int[][] ld = new int[m][2];
		for (int i = 0; i < m; i++) ld[i][0] = sc.nextInt();
		for (int i = 0; i < m; i++) ld[i][1] = sc.nextInt();
		sort(ld, comparingInt(a -> a[0]));
		long ans = 0;
