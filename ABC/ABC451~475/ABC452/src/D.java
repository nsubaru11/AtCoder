...
public final class D {
	...
	private static void solve() {
		...
		ArrayDeque<Integer>[] dq = new ArrayDeque[m];
		for (int i = 0; i < m; i++) dq[i] = new ArrayDeque<>();
		long ans = 0;
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				if (s[i] == t[j]) dq[j].add(i);
			}
		}
		for (int i = 0; i < n; i++) {
			int k = i - 1;
			for (ArrayDeque<Integer> dqj : dq) {
				while (!dqj.isEmpty() && dqj.peekFirst() <= k) dqj.pollFirst();
				if (dqj.isEmpty()) {
					k = n;
					break;
				}
