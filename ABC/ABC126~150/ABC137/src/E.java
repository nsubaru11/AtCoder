...
public final class E {
	...
	private static void solve() {
		...
		DirectedGraph graph2 = new DirectedGraph(n, m);
		for (int i = 0; i < m; i++) {
			int a = sc.nextInt0();
			int b = sc.nextInt0();
			int c = sc.nextInt() - p;
		...
		long[] ans = new long[n];
		fill(ans, -1_000_000_000_000L);
		ans[0] = 0;
		int cnt = 0;
		while (cnt < n) {
			boolean fi = true;
			for (int e = 0; e < m; e++) {
				if (edges[e] != 3) continue;
				int u = graph2.to(e), v = graph1.to(e);
				if (chmax(ans, v, graph1.cost(e) + ans[u])) {
					fi = false;
				}
			}
			if (fi) break;
		...
		out.println(cnt == n ? -1 : max(ans[n - 1], 0));
	}
