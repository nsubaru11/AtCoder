...
public class D {
	...
	public static void main(String[] args) {
		...
		w = sc.nextInt();
		int k = sc.nextInt();
		s = sc.nextCharMat(h, w);
		int cnt = 0;
		for (int i = 0; i < h; i++) {
			for (int j = 0; j < w; j++) {
				if (s[i][j] == '.') {
					cnt += count(i, j, k, new boolean[h][w]);
				}
			}
		}
	...
	private static int count(int i, int j, int k, boolean[][] used) {
		if (used[i][j]) {
			return 0;
		}
		if (k == 0) {
		...
		used[i][j] = true;
		if (i + 1 < h && s[i + 1][j] == '.') {
			cnt += count(i + 1, j, k - 1, used);
		}
		if (i - 1 >= 0 && s[i - 1][j] == '.') {
			cnt += count(i - 1, j, k - 1, used);
		}
		if (j + 1 < w && s[i][j + 1] == '.') {
			cnt += count(i, j + 1, k - 1, used);
		}
		if (j - 1 >= 0 && s[i][j - 1] == '.') {
			cnt += count(i, j - 1, k - 1, used);
		}
