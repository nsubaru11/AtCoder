...
public final class C {
	...
	private static void solve() {
		...
		for (int ha = 1; ha < h; ha++) {
			long a1 = (long) ha * w, b1 = (h - ha) / 2L * w, c1 = hw - a1 - b1;
			ans = min(ans, max(a1, b1, c1) - min(a1, b1, c1));
			long a2 = (long) ha * w, b2 = (h - ha) * (w / 2L), c2 = hw - a2 - b2;
			ans = min(ans, max(a2, b2, c2) - min(a2, b2, c2));
		}
		for (int wa = 1; wa < w; wa++) {
			long a3 = (long) wa * h, b3 = (w - wa) / 2L * h, c3 = hw - a3 - b3;
			ans = min(ans, max(a3, b3, c3) - min(a3, b3, c3));
			long a4 = (long) wa * h, b4 = (w - wa) * (h / 2L), c4 = hw - a4 - b4;
			ans = min(ans, max(a4, b4, c4) - min(a4, b4, c4));
		}
		out.println(ans);
	}
