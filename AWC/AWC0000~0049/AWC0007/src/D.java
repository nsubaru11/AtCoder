...
public final class D {
	...
	private static void solve() {
		...
		while (a-- > 0) {
			...
			int c2 = sc.nextInt0();
			tk[r1][c1]++;
			if (c2 + 1 < n) tk[r1][c2 + 1]--;
			if (r2 + 1 < n) tk[r2 + 1][c1]--;
			if (r2 + 1 < n && c2 + 1 < n) tk[r2 + 1][c2 + 1]++;
		}
		while (b-- > 0) {
			int r1 = sc.nextInt0();
			int c1 = sc.nextInt0();
			...
			int c2 = sc.nextInt0();
			ao[r1][c1]++;
			if (c2 + 1 < n) ao[r1][c2 + 1]--;
			if (r2 + 1 < n) ao[r2 + 1][c1]--;
			if (r2 + 1 < n && c2 + 1 < n) ao[r2 + 1][c2 + 1]++;
		}
		int cnt = 0;
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				if (i > 0 && j > 0) tk[i][j] = tk[i][j] - tk[i - 1][j - 1] + tk[i - 1][j] + tk[i][j - 1];
				else if (i > 0) tk[i][j] = tk[i][j] + tk[i - 1][j];
				else if (j > 0) tk[i][j] = tk[i][j] + tk[i][j - 1];
				if (i > 0 && j > 0) ao[i][j] = ao[i][j] - ao[i - 1][j - 1] + ao[i - 1][j] + ao[i][j - 1];
				else if (i > 0) ao[i][j] = ao[i][j] + ao[i - 1][j];
				else if (j > 0) ao[i][j] = ao[i][j] + ao[i][j - 1];
				if (tk[i][j] > 0 && ao[i][j] > 0) cnt++;
			}
		}
		out.println(cnt);
	}
