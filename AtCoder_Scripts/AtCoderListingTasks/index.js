...
`;
	const TEXT = {
		...
		caution: {
			'ja': `※一度に開くことのできるタブは ${ATONCE_TAB_MAX} 個までです。`,
			'en': `*Up to ${ATONCE_TAB_MAX} tabs are allowed to open at once.`
		},
		reverse: {'ja': '逆順で開く', 'en': 'Open in reverse order'},
		modalInfo: {
			'ja': 'が開きます。(ポップアップがブロックされた場合は許可してください。)',
			'en': 'will open. (If pop-ups are blocked, please allow them.)'
		},
		aTab: {'ja': '個のタブ', 'en': 'tab '},
	...
	const REMOVE_INTERVAL = 60 * 60 * 1000;   //1時間
			...
			if (this.dbExists) {
				...
				if (resArray[0] !== null) {
					...
					if (now - resArray[0].lastAccess >= ACCESS_INTERVAL) {
						setTasks.push(this.db.setData(STORE_NAME.problemList, {
	...
	Launcher.prototype = {
		...
		changeToDropdown: function () {
			...
			tasks_tab.parent().addClass('dropdown');
			tasks_tab.parent().append($('<ul>', {class: `dropdown-menu ${PRE}-dropdown`}));

			// マウスホバー時にドロップダウンを表示する設定
			tasks_tab.parent().on('mouseenter', function () {
				$(this).addClass('open');
		...
		addModal: function () {
			...
			this.isAll = this.setting.atOnce.begin === 0 && this.setting.atOnce.end === this.setting.problemList.length - 1;
			...
			let begin_button = $('<button>', {
				class: `btn btn-default dropdown-toggle ${PRE}-toggle`,
				'data-toggle': 'dropdown',
				'aria-expanded': 'false',
				text: 'A',
				disabled: this.isAll
			...
			//リストで選択したときの動作
			begin_list[0].addEventListener('click', {
				handleEvent: this.changeRange,
				that: this,
				begin_button,
				end_button,
				modalInfo,
			...
			end_list[0].addEventListener('click', {
				handleEvent: this.changeRange,
				that: this,
				begin_button,
				end_button,
			...
			let cancel = $('<button>', {
				...
				'data-dismiss': 'modal',
				text: TEXT.cancel[this.setting.lang]
			});
