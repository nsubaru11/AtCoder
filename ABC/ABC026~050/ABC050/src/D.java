...
public final class D {
	...
	private static void solve() {
		// test();
		long n = sc.nextLong();
		LongIntMap memo = new LongIntMap();
		memo.put(0, 1);
		memo.put(1, 2);
		out.println(recursion(n, memo));
	}

	private static int recursion(long n, LongIntMap memo) {
		if (memo.containsKey(n)) return memo.get(n);
		long k = n / 2;
		return memo.put(n, (int) ((2L * recursion(k - (1 - (n & 1)), memo) + recursion(k - (n & 1), memo)) % MOD));
	}
