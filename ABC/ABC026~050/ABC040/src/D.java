...
public final class D {
	...
	private static void solve() {
		...
		int[][] load = sc.nextIntMat(m, 3);
		sort(load, comparingInt(a -> a[2]));
		...
		sort(vw, comparingInt(a -> a[1]));
		int[] ans = new int[q];
		for (int l = m - 1, qi = q - 1; qi >= 0; qi--) {
			int v = vw[qi][0], w = vw[qi][1], i = vw[qi][2];
			while (l >= 0 && load[l][2] > w) {
				uf.union(load[l][0] - 1, load[l][1] - 1);
				l--;
			}
			ans[i] = uf.size(v);
		}
