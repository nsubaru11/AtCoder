...
public final class D {
	...
	private static void solve() {
		...
		parallelPrefix(cnt[0], Integer::sum);
		for (int i = 1; i <= k2 * 2; i++) cnt[i][0] += cnt[i - 1][0];
		...
		for (int i = 1; i <= k2; i++) {
			for (int j = 1; j <= k2; j++) {
				int current = cnt[i][j] + cnt[i + k2][j] + cnt[i][j + k2] + cnt[i + k2][j + k2];
