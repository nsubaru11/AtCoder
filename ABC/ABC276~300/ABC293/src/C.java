...
public final class C {
	...
	private static void solve() {
		...
		int w = sc.nextInt();
		int hw = h * w;
		int[] grid = sc.nextInt(hw);
		compression(grid, hw);
		out.println(dfs(0, h, w, hw, grid, new boolean[hw]));
	}

	private static int dfs(int u, int h, int w, int hw, int[] grid, boolean[] visited) {
		if (u == hw - 1) return 1;
		visited[grid[u]] = true;
		int ans = 0;
		for (int i = u / w, j = u % w, d = 2; d < 4; d++) {
			...
			int v = ni * w + nj;
			if (visited[grid[v]]) continue;
			ans += dfs(v, h, w, hw, grid, visited);
		}
		visited[grid[u]] = false;
		return ans;
	}
