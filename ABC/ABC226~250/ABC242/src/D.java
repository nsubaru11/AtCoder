...
public final class D {
	...
	private static void solve() {
		...
		while (q-- > 0) {
			long t = sc.nextLong();
			long k = sc.nextLong0();
			out.println((char) (recursion(t, k, s) + 'A'));
		}
	}

	private static int recursion(long t, long k, char[] s) {
		if (t == 0) return s[(int) k] - 'A';
		if (k == 0) return (int) ((s[0] - 'A' + t) % 3);
		return (int) ((recursion(t - 1, k / 2, s) + 1 + (k & 1)) % 3);
	}
