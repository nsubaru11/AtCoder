...
public final class A21 {
	...
	private static void solve() {
		...
		for (int r = 0; r < n; r++) {
			for (int l = r - 1; l >= 0; l--) {
				int left = isValidRange(p[l], l + 1, r + 1) ? a[l] : 0;
				int right = isValidRange(p[r], l, r) ? a[r] : 0;
				dp[l] = max(left + dp[l + 1], right + dp[l]);
			}
		}
		out.println(dp[0]);
	}
	...
	private static final class FastPrinter implements AutoCloseable {
		...
		private int write(int i, int p) {
			...
			int writePos = p + countDigits(i);
			final int end = writePos;
			while (i <= -100) {
				final int rem = -(i % 100);
				final short d = DIGITS_2[rem];
				buf[writePos - 1] = (byte) (d >>> 8);
				buf[writePos - 2] = (byte) d;
				i /= 100;
				writePos -= 2;
			}
			if (i <= -10) {
				final short d = DIGITS_2[-i];
				buf[writePos - 1] = (byte) (d >>> 8);
				buf[writePos - 2] = (byte) d;
			} else {
				buf[writePos - 1] = (byte) (ZERO - i);
			}
			return end;
		}
		...
		private int write(long l, int p) {
			...
			int writePos = p + countDigits(l);
			final int end = writePos;
			while (l <= -100L) {
				final int rem = (int) -(l % 100L);
				final short d = DIGITS_2[rem];
				buf[writePos - 1] = (byte) (d >>> 8);
				buf[writePos - 2] = (byte) d;
				l /= 100L;
			...
			if (l <= -10L) {
				final short d = DIGITS_2[(int) -l];
				buf[writePos - 1] = (byte) (d >>> 8);
				buf[writePos - 2] = (byte) d;
			} else {
				buf[writePos - 1] = (byte) (ZERO - l);
			}
			return end;
		}
