...
public final class F {
	...
	private static void solve() {
		...
		if (1 != rtN && n % rtN == 0) {
			factors.add(rtN);
			if (rtN * rtN != n) factors.add(n / rtN);
		}
		factors.sort(Integer::compareTo);
		int ans = 0;
		int[] must = new int[n];
		int[] a = new int[factors.size()];
		for (int fi = 0; fi < factors.size(); fi++) {
			int f = factors.get(fi);
			int cnt = 0;
			for (int i = 0; i < n; ) {
				for (int j = 0; j < f; i++, j++) {
					if (s[i] == '#' || must[j] == f) continue;
					must[j] = f;
					cnt++;
				}
			}
			int c = iModPow(2, f - cnt, MOD);
			for (int i = 0; i < fi; i++) {
				if (f % factors.get(i) == 0) c = (c - a[i]) % MOD;
			}
			if (c < 0) c += MOD;
			a[fi] = c;
			ans = (ans + c) % MOD;
