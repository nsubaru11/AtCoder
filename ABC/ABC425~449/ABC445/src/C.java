...
public final class C {
	...
	private static void solve() {
		int n = sc.nextInt();
		int[] a = new int[n];
		setAll(a, _ -> sc.nextInt() - 1);
		int[] loopLen = new int[n];
		int[] d = new int[n];
		int[] visited = new int[n];
		fill(visited, -1);
		ArrayList<int[]> loop = new ArrayList<>(n);
		int[] loopId = new int[n]; // 自分のループ番号
		int[] loopIdx = new int[n]; // 自分のインデックス
		int id = 0;
		int[] ans = new int[n];
		for (int i = 0; i < n; i++) {
			if (visited[i] == -1) {
				int[] list = dfs(i, 0, a, loopLen, d, visited, id, loopId, loopIdx);
				if (list != null) {
					loop.add(list);
					id++;
				}
			...
			int len = loopLen[i];
			ans[i] = loop.get(loopId[i])[((iModPow(10, 100, len) - d[i]) % len + len) % len] + 1;
		}
		out.print(ans).println();
	}

	private static int[] dfs(int i, int c, int[] a, int[] loopLen, int[] d, int[] visited, int id, int[] loopId, int[] loopIdx) {
		if (visited[i] != -1) {
			if (loopLen[i] != 0) {
				return null;
			} else {
				loopLen[i] = c - visited[i];
				d[i] = 0;
				int[] loop = new int[loopLen[i]];
				fill(loop, -1);
				loop[0] = i;
				loopId[i] = id;
				loopIdx[i] = 0;
		...
		int to = a[i];
		visited[i] = c;
		int[] loop = dfs(to, c + 1, a, loopLen, d, visited, id, loopId, loopIdx);
		if (loop != null) {
			int idx = loopLen[to] - d[to] - 1;
			if (idx >= 0 && loop[idx] == -1) {
				loop[idx] = i;
				loopIdx[i] = idx;
			}
