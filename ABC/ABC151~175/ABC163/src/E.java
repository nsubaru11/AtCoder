import lib.ds.priorityqueue.IntIndexedPriorityQueue;
...
public final class E {
    ...
    private static void solve() {
        int n = sc.nextInt();
        int[] a = sc.nextInt(n);
        IntIndexedPriorityQueue pq = new IntIndexedPriorityQueue(n, true);
        for (int i = 0; i < n; i++) pq.add(i, a[i]);
        long[] dp = new long[n + 1]; // 左端から i マス埋まっているときの最大スコア
        for (int c = 0; !pq.isEmpty(); c++) {
            int x = pq.peekIndex();
            long ax = pq.poll();
            dp[c + 1] = dp[c] + abs(c - x) * ax;
            for (int y = c; y >= 1; y--) {
                dp[y] += abs(n - (c - y) - 1 - x) * ax;
                chmax(dp, y, dp[y - 1] + abs(y - 1 - x) * ax);
            }
            dp[0] += abs(n - c - 1 - x) * ax;
        }
        out.println(max(dp));
