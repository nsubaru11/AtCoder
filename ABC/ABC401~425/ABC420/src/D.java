...
public final class D {
	...
	private static void solve() {
		...
		int[] visited = new int[hw * 2];
		int start = 0, goal = 0;
		for (int i = 0; i < hw; i++) {
			if (grid[i] == 'S') start = i;
			else if (grid[i] == 'G') goal = i;
		}
		...
		while (!dq.isEmpty()) {
			int u = dq.poll();
			boolean o = u < hw;
			int i = o ? u / w : (u - hw) / w, j = o ? u % w : (u - hw) % w;
			for (int d = 0; d < 4; d++) {
				int ni = i + di[d], nj = j + dj[d];
				if (!isValidRange(ni, nj, h, w)) continue;
				int vij = ni * w + nj;
				if (grid[vij] == '#' || (o && grid[vij] == 'x') || (!o && grid[vij] == 'o')) continue;
				int v = grid[vij] == '?' ? (o ? vij + hw : vij) : (!o ? vij + hw : vij);
				if (visited[v] != -1) continue;
				visited[v] = visited[u] + 1;
				if (vij == goal) break outer;
				dq.add(v);
			}
		}
		out.println(max(visited[goal], visited[goal + hw]));
	}
