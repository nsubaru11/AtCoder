...
public final class A {
	...
	private static final FastPrinter out = new FastPrinter(128);
	// endregion
	...
	private static void solve() {
		...
		char[] ans = new char[m];
		HashSet<Integer> exist = new HashSet<>(n);
		for (int i = 0; i < n; i++) exist.add(i);
		for (int j = 0; j < m; j++) {
			int cnt = 0;
			for (int i : exist) {
				if (s[i][j]) cnt++;
		...
		out.println(true).printChars(ans).println();
	}
