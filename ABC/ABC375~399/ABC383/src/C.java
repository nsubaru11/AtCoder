...
public class C {
	...
	public static void main(String[] args) {
		...
		Deque dq = new Deque(h * w);
		for (int i = 0; i < h; i++) {
			for (int j = 0; j < w; j++) {
				s[i][j] = sc.nextChar();
				if (s[i][j] == 'H') {
					dq.addLast(new Tuple(i, j));
				}
			}
		}
		int ans = 0;
		...
		while (!dq.isEmpty()) {
			...
			if (cnt < d) {
				for (int k = 0; k < 4; k++) {
					int di = i + dxy[k][0];
					int dj = j + dxy[k][1];
					if (di < h && dj < w && di >= 0 && dj >= 0) {
						if (!f[di][dj] && s[di][dj] == '.') {
							Tuple t = new Tuple(di, dj);
							t.cnt = cnt + 1;
							dq.addLast(t);
						}
					}
	...
	private static class Deque {
		...
		public Deque(int n) {
			size = n;
			list = new Tuple[size];
			setAll(list, i -> new Tuple(0, 0));
			len = head = tail = 0;
		}

		public Deque addLast(Tuple p) {
			if (isEmpty()) {
				head = tail = 0;
			} else {
				tail = (tail + 1) % size;
		...
		public Tuple peekLast() {
			return list[tail];
		}

		public Tuple peekFirst() {
			return list[head];
		}

		public Deque set(Tuple p, int idx) {
			list[(head + idx) % size] = p;
			return this;
		}
		...
		public Deque insert(Tuple p, int idx) {
			int i = tail;
			while (true) {
				list[(i + 1) % size] = list[i];
				if ((head + idx) % size == i) {
					list[i] = p;
					break;
				}
	...
	private static class Tuple {
		...
		public boolean equals(Object o) {
			if (o instanceof Tuple) {
				return i == ((Tuple) o).i && j == ((Tuple) o).j;
			}
			return false;
		}
	}
