...
public final class D {
	...
	private static void solve() {
		...
		out.print(topologicalSort(g), i -> i + 1).println();
	}

	private static int[] topologicalSort(DirectedGraph g) {
		int[] degree = new int[g.n];
		System.arraycopy(g.inDegree, 0, degree, 0, g.n);
		PriorityQueue<Integer> q = new PriorityQueue<>(comparingInt(i -> degree[(int) i]).thenComparingInt(i -> (int) i));
		for (int i = 0; i < g.n; i++) {
