...
public final class C {
	...
	private static void solve() {
		...
		int w = sc.nextInt();
		int hw = h * w;
		IntCompressedArray grid = new IntCompressedArray(sc.nextInt(hw));
		out.println(dfs(0, h, w, hw, grid, new boolean[grid.uniqueSize]));
	}

	private static int dfs(int u, int h, int w, int hw, IntCompressedArray grid, boolean[] visited) {
		if (u == hw - 1) return 1;
		visited[grid.get(u)] = true;
		int ans = 0;
		for (int i = u / w, j = u % w, d = 2; d < 4; d++) {
			int ni = i + di[d], nj = j + dj[d];
			if (!isValidRange(ni, nj, h, w)) continue;
			int v = ni * w + nj;
			if (visited[grid.get(v)]) continue;
			ans += dfs(v, h, w, hw, grid, visited);
		}
		visited[grid.get(u)] = false;
		return ans;
	}
