	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++) {
			for (int j = i + 1; j < n; j++) {
				...
				int iLen = a[i].length;
				int jLen = a[j].length;
				for (int k = 0, l = 0; k < iLen; k++) {
					while (l < jLen && a[i][k] > a[j][l]) {
						l++;
					}
					if (l < jLen && a[i][k] == a[j][l]) {
						long cnt1 = 1;
						while (k + 1 < iLen && a[i][k] == a[i][k + 1]) {
							k++;
							cnt1++;
						}
						long cnt2 = 1;
						while (l + 1 < jLen && a[j][l] == a[j][l + 1]) {
							l++;
							cnt2++;
						}
						cnt += cnt1 * cnt2;
				...
				double tmp = (double) cnt / ((long) iLen * jLen);
				max = max(max, tmp);
			}
		}
		out.println(max);
