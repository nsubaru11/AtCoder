...
public final class C {
	...
	private static void solve() {
		int ha = sc.nextInt();
		int wa = sc.nextInt();
		boolean[][] a = nextGrid(ha, wa);

		int hb = sc.nextInt();
		int wb = sc.nextInt();
		boolean[][] b = nextGrid(hb, wb);

		int hx = sc.nextInt();
		int wx = sc.nextInt();
		boolean[][] x = nextGrid(hx, wx);

		boolean[][] c = new boolean[hx][wx];
		int h = max(ha, hb, hx), w = max(wa, wb, wx);
		for (int ia = -h; ia < hx; ia++) {
			for (int ja = -w; ja < wx; ja++) {
				for (int ib = -h; ib < hx; ib++) {
					outer:
					for (int jb = -w; jb < wx; jb++) {
						for (int i = 0; i < hx; i++) fill(c[i], false);
						for (int i1 = ia, i = 0; i < ha; i1++, i++) {
							for (int j1 = ja, j = 0; j < wa; j1++, j++) {
								boolean f = isValidRange(i1, j1, hx, wx);
								if (a[i][j] && !f) continue outer;
								if (!f) continue;
								c[i1][j1] |= a[i][j];
							}
						}
						for (int i1 = ib, i = 0; i < hb; i1++, i++) {
							for (int j1 = jb, j = 0; j < wb; j1++, j++) {
								boolean f = isValidRange(i1, j1, hx, wx);
								if (b[i][j] && !f) continue outer;
								if (!f) continue;
								c[i1][j1] |= b[i][j];
							}
						}
	...
	private static boolean[][] nextGrid(final int h, final int w) {
