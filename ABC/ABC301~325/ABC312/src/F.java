...
public final class F {
	...
	private static void solve() {
		...
		sort(can1, 0, c1);
		sort(can2, 0, c2, (o1, o2) -> o1[0] != o2[0] ? o1[0] - o2[0] : o1[1] - o2[1]);
		sort(key, 0, k);
		long ans = 0, sum = 0;
		int len = min(m, c1);
		for (int i = 0; i < len; i++) sum += can1[c1 - i - 1];
		if (len == m) ans = sum;
		IntPriorityQueue pq = new IntPriorityQueue(m);
		for (int i2 = c2 - 1, ki = k - 1; i2 >= 0; i2--) {
