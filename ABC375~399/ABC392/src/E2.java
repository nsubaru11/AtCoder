...
public class E2 {
	...
	public static void main(String[] args) {
		...
		List<Pair> pairList = new ArrayList<>(m);
		for (int i = 0; i < m; i++) {
			int a = sc.nextInt() - 1;
			int b = sc.nextInt() - 1;
			if (nodes.isConnected(a, b)) {
				pairList.add(new Pair(i, a, b));
			} else {
				int findA = nodes.find(a);
				int findB = nodes.find(b);
				nodes.union(a, b);
				if (findA == nodes.find(a)) {
					roots.remove(findB);
				} else {
					roots.remove(findA);
				}
		...
		out.ensureCapacity(nodes.groupCount() * 6);
		out.println(nodes.groupCount() - 1);
		int i = 0;
		int a = nodes.find(0);
		for (int r: roots) {
			if (r == a) continue;
			Pair pair = pairList.get(i);
			if (nodes.isConnected(pair.a, r)) {
				nodes.union(a, pair.a);
				out.print(pair.i + 1 + " ");
				out.print(pair.a + 1 + " ");
				out.println(1);
			} else {
				nodes.union(pair.a, r);
				out.print(pair.i + 1 + " ");
				out.print(pair.a + 1 + " ");
				out.println(r + 1);
			}
