...
public class E {
	...
	private static void solve(final FastScanner sc, final FastPrinter out) {
		 ...
		 * リンゴ | バナナ | ブドウ
		 *     オレンジ     |
		 * このときの並べ方は、リンゴとバナナの間に オレンジを bi 個 入れる入れ方
		 * 0 <= i <= a + c + 1
		 * b0 + b1 + b2 + ... + b_a+c+1 = b
		 * a+c+1種類から重複を許してb個選ぶ方法
		 * C(a+b+c, b)
		 * バナナr > ブドウl の時
		 * リンゴ | バナナ, ブドウ
		 *     オレンジ     |
		 * バナナc - i 種類から重複を許してd - 1個を選ぶ方法
		 * C(c - i - 1 + d - 1 - 1, d - 1) = C(c - i + d - 1 - 1, d - 1) * (c - i - 1) / (c - i + d - 1 - 1, d - 1)
		 * i >= 1
		 */
		long ans = comb(a + b + c, b, 998244353);
		long temp = comb(c + d - 2, d - 1, 998244353);
		for (int i = 1; i <= c; i++) {
			temp = temp * (c - i - 1);
//			ans += comb(temp + a + b, );
		}
	 ...
	 * @return nCr
	 */
	public static long comb(int n, int r, int mod) {
