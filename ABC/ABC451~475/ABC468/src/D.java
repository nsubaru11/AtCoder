...
public final class D {
	...
	private static void solve() {
		...
		int ans = 0;
		for (int i = 0; i < n; i++) {
			int k = manacher.getOddPalindromeLengthAt(i) + 1 >> 1;
			ans += k;
			boolean flag = false;
			for (int l = i - k, r = i + k; l >= 0 && r < n; l--, r++) {
				if (s[l] != s[r]) {
					if (flag) break;
					flag = true;
				}
		...
		for (int i = 0; i < n - 1; i++) {
			int k = manacher.getEvenPalindromeLengthAt(i) >> 1;
			ans += k;
			boolean flag = false;
			for (int l = i - k, r = i + k + 1; l >= 0 && r < n; l--, r++) {
				if (s[l] != s[r]) {
					if (flag) break;
					flag = true;
				}
