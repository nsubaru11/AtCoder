...
public final class C {
	...
	private static void solve() {
		...
		int k = sc.nextInt();
		int m = sc.nextInt();
		IntPriorityQueue beginner = new IntPriorityQueue(n, true);
		IntPriorityQueue expert = new IntPriorityQueue(n, true);
		while (n-- > 0) {
			int h = sc.nextInt();
			int p = sc.nextInt();
			if (h == 0) beginner.push(p);
			else expert.push(p);
		}
		if (expert.size() < m || beginner.size() < k - m) {
			out.println(-1);
			return;
		}
		long ans = 0;
