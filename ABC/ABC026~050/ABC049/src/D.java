...
public final class D {
	...
	private static void solve() {
		...
		while (k-- > 0) {
			int p = sc.nextInt0(), q = sc.nextInt0();
			load.union(p, q);
		}
		while (l-- > 0) {
			int r = sc.nextInt0(), s = sc.nextInt0();
			train.union(r, s);
		}
		IntPairIntMap map = new IntPairIntMap(n);
		for (int i = 0; i < n; i++) {
			map.increment(load.find(i), train.find(i));
		}
		int[] ans = new int[n];
		for (int i = 0; i < n; i++) {
			ans[i] = map.get(load.find(i), train.find(i));
		}
