...
public final class D {
	...
	private static final FastPrinter out = new FastPrinter(256);
	// endregion
	...
	private static void solve() {
		...
		int n = sc.nextInt();
		int[][] ab = new int[2][n];
		IntPairIntMap black = new IntPairIntMap(n);
		for (int i = 0; i < n; i++) {
			int a = ab[0][i] = sc.nextInt0(), b = ab[1][i] = sc.nextInt0();
			black.add(a, b, 1);
		}
		IntPairIntMap[] hs = new IntPairIntMap[10];
		setAll(hs, _ -> new IntPairIntMap(n));
		for (int qi = 0; qi < n; qi++) {
			int a = ab[0][qi], b = ab[1][qi];
			for (int ci = 0, i = a - 2; ci < 3; ci++, i++) {
				outer:
				for (int cj = 0, j = b - 2; cj < 3; cj++, j++) {
					int cnt = 0;
					for (int di = 0; di < 3; di++) {
						for (int dj = 0; dj < 3; dj++) {
							int ni = i + di, nj = j + dj;
							if (!isValidRange(ni, nj, h, w)) continue outer;
							cnt += black.getOrDefault(ni, nj, 0);
						}
					}
					hs[cnt].add(i, j, 1);
				}
		...
		long sum = 0;
		for (var hsi : hs) sum += hsi.size();
		out.println((h - 2) * (w - 2L) - sum);
		for (int i = 1; i < 10; i++) out.println(hs[i].size());
	}
	...
	private static void debugln(final Object... args) {
		if (DEBUG) {
			...
			else System.err.println(stream(args).map(D::stringify).collect(Collectors.joining("\n", "\n", "")));
		}
	}
