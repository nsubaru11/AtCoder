...
public final class F {
	...
	private static void solve() {
		...
		for (int i = 0; i < n; i++) {
			int top = 0;
			for (int j = 0; j < m; j++) {
				while (top > 0 && s[i][stack[top - 1]] >= s[i][j]) {
					int ht = s[i][stack[--top]];
					int l = top == 0 ? -1 : stack[top - 1];
					int hl = max(l == -1 ? 0 : s[i][l], s[i][j]) + 1;
					int w = j - l - 1;
					int hi = max(hl - 1, min(ht, k / w));
					ans += (w + 1L) * w / 2 * (hi - hl + 1);
					ans += (w + 1) * (sum1[ht] - sum1[hi]) - (sum2[ht] - sum2[hi]);
				}
				stack[top++] = j;
			}
			while (top > 0) {
