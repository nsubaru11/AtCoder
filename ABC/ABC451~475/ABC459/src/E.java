...
public final class E {
	...
	private static void solve() {
		...
		long[] c = sc.nextLong(n);
		int[] d = sc.nextInt(n);
		FactorialTable table = new FactorialTable(max(d));
		int[] q = new int[n];
		int tail = 0;
		for (int i = 0; i < n; i++) {
			if (deg[i] == 0) q[tail++] = i;
		...
		for (int head = 0; head < tail; head++) {
			...
			ans = ans * numer(c[u], d[u]) % MOD * table.invFact(d[u]) % MOD;
			c[v] += c[u] - d[u];
			if (--deg[v] == 0 && u != 0) q[tail++] = v;
		}
		out.println(ans);
	}

	private static long numer(long n, int r) {
		long res = 1;
		for (int i = 0; i < r; i++) {
			res = res * ((n - i) % MOD) % MOD;
		}
