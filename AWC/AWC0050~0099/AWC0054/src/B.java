...
public final class B {
	...
	private static void solve() {
		int n = sc.nextInt();
		int w = sc.nextInt();
		int[][] lr = sc.nextIntMat(n, 2);
		sort(lr, comparingInt(a -> a[0]));
		int l = lr[0][0] - 1, r = l + lr[0][1] - 1;
		int ans = 0;
		for (int i = 1; i < n; i++) {
			int li = lr[i][0] - 1, ri = li + lr[i][1] - 1;
			if (isValidRange(li, l, r + 2)) {
				r = max(r, ri);
			} else {
				ans += r - l + 1;
				l = li;
