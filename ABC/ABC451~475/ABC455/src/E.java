...
public final class E {
	...
	private static void solve() {
		...
		IntTripleIntMap abc = new IntTripleIntMap(n + 1);
		IntPairIntMap ab = new IntPairIntMap(n + 1);
		IntPairIntMap ac = new IntPairIntMap(n + 1);
		IntPairIntMap bc = new IntPairIntMap(n + 1);
		abc.put(0, 0, 0, 1);
		ab.put(0, 0, 1);
		ac.put(0, 0, 1);
		bc.put(0, 0, 1);
		...
		for (int i = 1; i <= n; i++) {
			...
			int mn = min(pa, pb, pc);
			int da = pa - mn, db = pb - mn, dc = pc - mn;
			int dab = min(da, db), dac = min(da, dc), dbc = min(db, dc);

			int tabc = abc.getOrDefault(da, db, dc, 0);
			int tab = ab.getOrDefault(da - dab, db - dab, 0);
			int tac = ac.getOrDefault(da - dac, dc - dac, 0);
			int tbc = bc.getOrDefault(db - dbc, dc - dbc, 0);

			ans -= tab + tac + tbc - tabc * 2L;

			abc.increment(da, db, dc);
			ab.increment(da - dab, db - dab);
			ac.increment(da - dac, dc - dac);
			bc.increment(db - dbc, dc - dbc);
		}
		out.println(ans);
	}
	...
	private static final class IntPairIntMap {
	...
	@SuppressWarnings("unused")
	private static final class BaseLongIntMap {
		private long[] keys;
	...
	@SuppressWarnings("unused")
	private static final class IntTripleIntMap {
