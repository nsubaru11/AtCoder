...
public final class C {
	...
	private static void solve() {
		...
		int ax = 0, ay = 0, ah = 0;
		for (int i = 0; i < n; i++) {
			int x = xyh[0][i] = sc.nextInt();
			int y = xyh[1][i] = sc.nextInt();
			int h = xyh[2][i] = sc.nextInt();
			if (h > 0) {
				ax = x;
				ay = y;
				ah = h;
		...
		for (int cx = 0; cx <= 100; cx++) {
			...
			for (int cy = 0; cy <= 100; cy++) {
				int ch = abs(cx - ax) + abs(cy - ay) + ah;
				for (int i = 0; i < n; i++) {
					int x = xyh[0][i], y = xyh[1][i], h = xyh[2][i];
					if (h == 0) {
						int th = max(ch - abs(cx - x) - abs(cy - y), 0);
						if (th != h) continue outer;
					} else {
						int th = abs(cx - x) + abs(cy - y) + h;
						if (ch != th) continue outer;
					}
				}
