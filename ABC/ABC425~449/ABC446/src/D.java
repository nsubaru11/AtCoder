...
public final class D {
	...
	private static void solve() {
		int n = sc.nextInt();
		BaseIntIntMap hm = new BaseIntIntMap(n);
		int ans = 0;
		while (n-- > 0) {
			final int v = sc.nextInt();
			int l = hm.getOrDefault(v - 1, 0);
			ans = max(ans, hm.merge(v, l + 1, Math::max));
		}
		out.println(ans);
	}
	...
	private static final class BaseIntIntMap {
		...
		private static int normalizeCapacity(int c) {
			long required = (long) c * 4 / 3 + 1;
		...
		public void forEach(BaseIntIntMap.IntIntConsumer action) {
