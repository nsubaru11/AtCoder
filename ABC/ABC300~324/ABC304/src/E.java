...
public final class E {
	...
	private static void solve() {
		...
		int k = sc.nextInt();
		HashSet<Long> bad = new HashSet<>(k * 2);
		while (k-- > 0) {
			int x = sc.nextInt0();
			int y = sc.nextInt0();
			int rx = uf.find(x), ry = uf.find(y);
			bad.add((long) rx * n + ry);
			bad.add((long) ry * n + rx);
		}
		int q = sc.nextInt();
		while (q-- > 0) {
			int rp = uf.find(sc.nextInt0());
			int rq = uf.find(sc.nextInt0());
			out.println(!bad.contains((long) rp * n + rq));
		}
	}
