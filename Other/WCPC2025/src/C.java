...
public final class C {
	...
	private static void solve() {
		...
		for (int i = 0; i < a; i++) fill(memo[i], -1);
		memo[a - 1][b - 1] = 1;
		while (k-- > 0) {
			int x = sc.nextInt() - 1;
			int y =sc.nextInt() - 1;
	...
	private static int dfs(int x, int y, int a, int b, int[][] memo) {
		if (memo[x][y] != -1) return memo[x][y];
		int xd = 0, yd = 0;
		if (x + 1 < a) xd = dfs(x + 1, y, a, b, memo);
		if (y + 1 < b) yd = dfs(x, y + 1, a, b, memo);
		return memo[x][y] = xd + yd;
	}
