...
public final class D {
	...
	private static void solve() {
		int n = sc.nextInt();
		int[] pow10 = new int[11];
		setAll(pow10, i -> i > 0 ? pow10[i - 1] * 10 : 1);
		IntAVLSet ts = new IntAVLSet();
		int[] pow2 = new int[30];
		setAll(pow2, i -> 1 << i);
		int[] pow2Digit = new int[30];
		setAll(pow2Digit, i -> digitCount(1 << i));
		dfs(pow10, pow2, pow2Digit, ts, 0);
		out.println(ts.getByIndex(n - 1));
	}

	private static void dfs(int[] pow10, int[] pow2, int[] pow2Digit, IntAVLSet ts, int a) {
		int da = a == 0 ? 0 : digitCount(a);
		for (int i = 0; i < 30; i++) {
			if (da + pow2Digit[i] >= 10) break;
			int next = a * pow10[pow2Digit[i]] + pow2[i];
			ts.add(next);
			dfs(pow10, pow2, pow2Digit, ts, next);
		}
	}

	private static int digitCount(final int x) {
		int i = x > 0 ? -x : x;
		if (i > -100000) {
			if (i > -100) {
				return i > -10 ? 1 : 2;
	...
	private static final class IntAVLSet implements Iterable<Integer> {
		// -------------- Fields --------------
		private Node root;
