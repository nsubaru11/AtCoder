...
public final class C {
	...
	private static void solve() {
		...
		out.println(dfs(0, new int[m], c, a, x, n, m));
	}

	private static int dfs(int i, int[] level, int[] c, int[][] a, int x, int n, int m) {
		if (i == n) {
			for (int l : level) if (l < x) return -1;
			return 0;
		}
		int l = dfs(i + 1, level, c, a, x, n, m);
		for (int j = 0; j < m; j++) level[j] += a[i][j];
		int r = dfs(i + 1, level, c, a, x, n, m);
		for (int j = 0; j < m; j++) level[j] -= a[i][j];
		if (l == -1 && r == -1) return -1;
		else if (l == -1) return r + c[i];
		else if (r == -1) return l;
		else return min(l, r + c[i]);
	}
