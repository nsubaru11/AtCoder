...
public final class E {
	...
	private static void solve() {
		...
		int[] dist1 = new int[n];
		int[] dist2 = new int[n];
		int u = bfs(0, tree, dist1, q);
		int v = bfs(u, tree, dist1, q);
		bfs(v, tree, dist2, q);
		int mx = max(u, v) + 1;
		for (int i = 0; i < n; i++) {
			if (dist1[i] > dist2[i]) {
	...
	private static int bfs(int u, Tree tree, int[] dist, int[] q) {
		fill(dist, -1);
		dist[u] = 0;
		q[0] = u;
		int mx = u;
		for (int head = 0, tail = 1, n = tree.n(); tail < n; head++) {
			u = q[head];
			for (final int v : tree.adj(u)) {
				if (dist[v] != -1) continue;
				dist[v] = dist[u] + 1;
				q[tail++] = v;
				if (dist[mx] < dist[v] || (dist[mx] == dist[v] && mx < v)) mx = v;
			}
		}
		return mx;
	}
