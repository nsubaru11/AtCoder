...
public final class D {
	...
	private static void solve() {
		int n = sc.nextInt();
		int[][] wh = sc.nextIntMat(n, 2);
		sort(wh, (o1, o2) -> o1[0] != o2[0] ? Integer.compare(o1[0], o2[0]) : Integer.compare(o2[1], o1[1]));
		out.println(lis(wh, n));
	}

	private static int lis(int[][] wh, int n) {
		int[] dpa = new int[n];
		int len = 0;
		for (int i = 0; i < n; i++) {
			int va = wh[i][1];
			int pos = binarySearch(dpa, 0, len, va);
			if (pos < 0) {
				pos = ~pos;
				dpa[pos] = va;
			}
