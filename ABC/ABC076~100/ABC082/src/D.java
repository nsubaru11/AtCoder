...
public final class D {
	...
	private static void solve() {
		...
		HashSet<Integer> dpX = new HashSet<>();
		dpX.add(s[0] == 'F' ? mx[0] : 0);
		for (int i = s[0] == 'F' ? 1 : 0; i < xl; i++) {
			HashSet<Integer> dp = new HashSet<>(dpX.size() * 2);
			int v = mx[i];
			dpX.forEach(k -> {
				dp.add(k + v);
				dp.add(k - v);
		...
		HashSet<Integer> dpY = new HashSet<>();
		dpY.add(0);
		for (int i = 0; i < yl; i++) {
			HashSet<Integer> dp = new HashSet<>(dpY.size() * 2);
			int v = my[i];
			dpY.forEach(k -> {
				dp.add(k + v);
				dp.add(k - v);
		...
		out.println(dpX.contains(x) && dpY.contains(y));
	}
