...
public final class E {
	...
	private static void solve() {
		...
		for (int i = 0; i < n; i++) fill(dist[i], Long.MAX_VALUE / 2);
		int[][] abc = new int[3][m];
		for (int i = 0; i < m; i++) {
			int a = abc[0][i] = sc.nextInt0();
			int b = abc[1][i] = sc.nextInt0();
			int c = abc[2][i] = sc.nextInt();
			dist[a][b] = dist[b][a] = c;
		}
		for (int k = 0; k < n; k++) {
			for (int i = 0; i < n; i++) {
		...
		int ans = 0;
		for (int e = 0; e < m; e++) {
			int a = abc[0][e], b = abc[1][e], c = abc[2][e];
			int unused = 0;
			for (int i = 0; i < n; i++) {
