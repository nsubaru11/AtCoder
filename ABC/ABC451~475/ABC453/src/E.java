...
public final class E {
	...
	private static void solve() {
		...
		for (int i = 0; i < n; i++) {
			...
			cntA[l]++;
			cntA[r + 1]--;
			int pl = max(l, n - r), pr = min(r, n - l);
			if (pl <= pr) {
				cntB[pl]++;
				cntB[pr + 1]--;
			}
		}
		parallelPrefix(cntA, Integer::sum);
		parallelPrefix(cntB, Integer::sum);
		FactorialTable table = new FactorialTable(n);
		int ans = 0;
		for (int i = 1; i < n; i++) {
			int c = cntB[i], l = cntA[i] - c, r = cntA[n - i] - c;
			if (l + r + c != n || l > i || n - i < r) continue;
			ans = (ans + table.nCr(c, i - l)) % MOD;
		}
		out.println(ans);
	}
	...
	private static final class FactorialTable {
