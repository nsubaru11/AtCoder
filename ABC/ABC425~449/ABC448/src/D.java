...
public final class D {
	...
	private static void solve() {
		...
		fill(ans, true);
		dfs(tree, a, hs, ans, 0, -1);
		out.println(ans);
	}

	private static void dfs(UndirectedGraph tree, int[] a, HashSet<Integer> hs, boolean[] ans, int u, int p) {
		if (!hs.add(a[u])) return;
		ans[u] = false;
		for (int v : tree.adj(u)) {
			if (v == p) continue;
			dfs(tree, a, hs, ans, v, u);
		}
		hs.remove(a[u]);
	}
