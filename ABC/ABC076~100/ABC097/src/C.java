...
public final class C {
	...
	private static void solve() {
		...
		for (int i = 0, h = 0; i < n; i++) {
			for (int j = 0; i + j < n && j <= k; j++) {
				if (!insert(q, h, i, i + j, s, n)) continue;
				if (h < k) h++;
			}
		}
		out.printChars(s, q[k - 1] / n, q[k - 1] % n + 1);
	}

	private static boolean insert(int[] q, int h, int i, int j, char[] s, int n) {
		int ij = i * n + j;
		int p = 0;
		for (int k = h - 1; k >= 0; k--) {
			int c = compare(s, q[k], ij, n);
			if (c == 0) return false;
			else if (c < 0) {
				p = k + 1;
				break;
		...
		if (p < q.length) {
			for (int k = h < q.length ? h : h - 1; k > p; k--) {
				q[k] = q[k - 1];
			}
			q[p] = ij;
			return true;
	...
	private static int compare(char[] a, int i, int j, int n) {
		int li = i / n, ri = i % n, iLen = ri - li + 1;
		int lj = j / n, rj = j % n, jLen = rj - lj + 1;
		for (int k = 0; k < min(iLen, jLen); k++) {
			if (a[li + k] != a[lj + k]) return a[li + k] - a[lj + k];
		}
		return iLen - jLen;
	}
	...
	private static void debugln(final Object... args) {
		if (DEBUG) {
			out.flush();
			System.err.println(stream(args).map(C::stringify).collect(Collectors.joining("\n", "\n", "")));
		}
	}
