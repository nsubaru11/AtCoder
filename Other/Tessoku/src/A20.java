...
public final class A20 {
	...
	private static void solve() {
		...
		for (char si : s) {
			for (int i = 0, max = 0; i < m; i++) {
				int temp = max;
				if (max < lcs[i]) max = lcs[i];
				if (si == t[i]) lcs[i] = temp + 1;
			}
		}
		out.println(max(lcs));
	}
	...
	private static final class FastScanner {
		private static final int MIN_CAPACITY = 1 << 20, SENTINEL_MARGIN = 64;
		private static final ValueLayout.OfByte JAVA_BYTE = ValueLayout.JAVA_BYTE;
		private static final ValueLayout.OfLong LONG_UNALIGNED = ValueLayout.JAVA_LONG_UNALIGNED.withOrder(ByteOrder.LITTLE_ENDIAN);
		...
		public FastScanner(final InputStream in) {
			try {
				int capacity = max(MIN_CAPACITY, in.available() + SENTINEL_MARGIN);
				ByteBuffer buf = ByteBuffer.allocateDirect(capacity);
				ReadableByteChannel channel = Channels.newChannel(in);
				while (channel.read(buf) != -1) {
					if (buf.capacity() - buf.position() < SENTINEL_MARGIN) {
						ByteBuffer newBuf = ByteBuffer.allocateDirect(buf.capacity() << 1);
						buf.flip();
						newBuf.put(buf);
						buf = newBuf;
					}
				...
				buf.flip();
				bufferLength = buf.limit();
				buf.clear();
				segment = MemorySegment.ofBuffer(buf);
		...
		public boolean hasNext() {
			while (pos < bufferLength) {
				if (segment.get(JAVA_BYTE, pos) > 32) return true;
				pos++;
			}
			return false;
		}
		...
		public char nextChar() {
			...
			return (char) segment.get(JAVA_BYTE, pos++);
		}
		...
		public int nextInt() {
			...
			boolean negative = b == '-';
			int p = pos, n = 0;
			if (negative) b = segment.get(JAVA_BYTE, p++);
			long a = segment.get(LONG_UNALIGNED, p - 1) ^ 0x3030303030303030L;
		...
		public long nextLong() {
			...
			if (check == 0) {
				...
				p += 7;
				b = segment.get(JAVA_BYTE, p++);
				long a2 = segment.get(LONG_UNALIGNED, p - 1) ^ 0x3030303030303030L;
		...
		public double nextDouble() {
			...
			if (b == '.') result += parseFracPart(p, segment);
			else pos = p;
			return negative ? -result : result;
		}

		private double parseFracPart(int p, final MemorySegment segment) {
			int b = segment.get(JAVA_BYTE, p++);
		...
		public String next() {
			...
			int p = pos;
			final int start = p - 1;
			while (segment.get(JAVA_BYTE, p) > 32) p++;
			final String s = new String(segment.asSlice(start, p - start).toArray(JAVA_BYTE), StandardCharsets.US_ASCII);
			pos = p + 1;
			return s;
		}
		...
		public String nextLine() {
			...
			while (p < bufferLength) {
				final int b = segment.get(JAVA_BYTE, p);
				if (b == '\n' || b == '\r') break;
				p++;
			}
			pos = p + (segment.get(JAVA_BYTE, p) == '\r' && segment.get(JAVA_BYTE, p + 1) == '\n' ? 2 : 1);
			return new String(segment.asSlice(start, p - start).toArray(JAVA_BYTE), StandardCharsets.US_ASCII);
		}
		...
		public char[] nextChars() {
			...
			for (int i = 0; i < len; i++) {
				c[i] = (char) segment.get(JAVA_BYTE, start + i);
			}
			pos = p + 1;
			return c;
		}
