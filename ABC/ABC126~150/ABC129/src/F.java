...
public final class F {
	...
	private static void solve() {
		...
		long al = a + b * (l - 1);
		int d10 = digit10(al);
		long[] pow10 = new long[d10 + 1];
		pow10[0] = 1;
		for (int i = 1; i <= d10; i++) pow10[i] = pow10[i - 1] * 10;
		long[] r = new long[d10 + 1];
		int da = digit10(a);
		for (int d = da; d < d10; d++) {
			r[d] = (pow10[d] - a - 1) / b;
		}
		r[da - 1] = -1;
		r[d10] = l - 1;
		long[] res = {0, a % m, 1};
		for (int d = da; d <= d10; d++) {
			long[][] x = {{pow10[d], 1, 0}, {0, 1, b % m}, {0, 0, 1}};
			LongMatrixUtils.modPowRaw(x, r[d] - r[d - 1], m);
			LongMatrixUtils.modMulRaw(x, res, m);
		}
		out.println(res[0]);
	}
	...
	private static final class LongMatrixUtils {
		...
		public static void modAffineRaw(final long[][] a, long x, long y, final long mod) {
			...
			y %= mod;
			for (int i = 0; i < a.length; i++)
				for (int j = 0; j < a[0].length; j++) a[i][j] = (x * a[i][j] % mod + y) % mod;
		}
