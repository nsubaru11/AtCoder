...
public final class C {
	...
	private static void solve() {
		int n = sc.nextInt();
		int[] l = sc.nextInt(n);
		out.println(dfs(0, 0, l, n));
	}

	private static int dfs(int i, long p, int[] l, int n) {
		if (i == n) return 0;
		long pl = p - l[i], pr = p + l[i];
		int a = (p >= 0 && pl < 0 ? 1 : 0) + dfs(i + 1, pl, l, n);
		int b = (p < 0 && pr >= 0 ? 1 : 0) + dfs(i + 1, pr, l, n);
		return max(a, b);
	}
