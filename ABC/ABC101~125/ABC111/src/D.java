...
public final class D {
	...
	private static void solve() {
		...
		boolean isEven = (abs(xy[0][0]) + abs(xy[0][1]) & 1) == 0;
		if (stream(xy).anyMatch(a -> isEven != ((abs(a[0]) + abs(a[1]) & 1) == 0))) {
			out.println(-1);
			return;
		}
		int m = stream(xy).mapToInt(a -> abs(a[0]) + abs(a[1])).max().orElse(0);
		m = digit2(m) + (isEven ? 1 : 0);
		int[] d = new int[m];
		if (isEven) {
			d[0] = 1;
			for (int i = 1; i < m; i++) {
