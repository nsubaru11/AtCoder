...
public final class E {
	...
	private static void solve() {
		...
		while (t-- > 0) {
			...
			int w = sc.nextInt();
			char[][] s = sc.nextCharMat(n, w);
			DirectedGraph g = new DirectedGraph(n * w, m * w * 2 + n * w);
			for (int d1 = 0; d1 < w; d1++) {
				int d2 = (d1 + 1) % w;
				for (int e = 0, j = 0; j < m; j++, e += 2) {
					int u = edge[e], v = edge[e + 1];
					if (s[u][d1] == 'o' && s[v][d2] == 'o') {
						g.add(u * w + d1, v * w + d2);
					}
					if (s[v][d1] == 'o' && s[u][d2] == 'o') {
						g.add(v * w + d1, u * w + d2);
					}
				}
			}
