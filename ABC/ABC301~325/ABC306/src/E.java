...
public final class E {
	...
	private static void solve() {
		...
		int k = sc.nextInt();
		int q = sc.nextInt();
		IndexedPriorityQueue topK = new IndexedPriorityQueue(n);
		for (int i = 0; i < k; i++) topK.push(i, 0);
		IndexedPriorityQueue polled = new IndexedPriorityQueue(n, true);
		for (int i = k; i < n; i++) polled.push(i, 0);
		long sum = 0;
		while (q-- > 0) {
			int x = sc.nextInt0();
			int y = sc.nextInt();
			if (topK.contains(x)) {
				sum -= topK.getCost(x) - y;
				topK.updateCost(x, y);
				if (!polled.isEmpty() && polled.peek() > y) {
					int i = polled.peekNode();
					int p = (int) polled.poll();
					sum += p - y;
					topK.push(i, p);
					topK.remove(x);
					polled.push(x, y);
