...
public final class E {
	...
	private static void solve() {
		...
		int k = sc.nextInt();
		int[] a = sc.nextInt(n);
		// ある区間の和をkで割った余りがその区間の要素数になるかどうか
		// ある区間の和をkで割った余りをf(l, r)と定める
		// min(n, k) < f(l, r)となる区間は無視して良い
		// 求めたいのは、f(l, r) == r - l + 1 となるl, r式変形or片方を固定
		// f(l, r) + l == r + 1 みたいな変形をする？
		// Σ a_i [l, r] + l == r + 1
		// 左側を固定したときに、右端が決定できる？
		// 違う。式変形がよくない
		// Σ a_i [0, r] - Σ a_i [0, l - 1]  == r - l + 1
		// Σ a_i [0, r] - r == Σ a_i [0, l - 1] - l + 1
		HashMap<Integer, Integer> hm = new HashMap<>(n);
		int[] keys = new int[n];
		long ans = 0;
		int window = min(n, k - 1);
		for (int i = 0, ps = 0; i < n; i++) {
			int key = ((ps - i + 1) % k + k) % k;
			keys[i] = key;
			hm.merge(key, 1, Integer::sum);
			if (i - window >= 0) hm.compute(keys[i - window], (_, v) -> v == 1 ? null : v - 1);
			ans += hm.getOrDefault((((ps + a[i]) % k - i) % k + k) % k, 0);
			ps = (ps + a[i]) % k;
		}
		out.println(ans);
	}
