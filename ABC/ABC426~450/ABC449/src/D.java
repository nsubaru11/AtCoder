...
public final class D {
	...
	private static void solve() {
		...
		int d = sc.nextInt();
		int u = sc.nextInt();
		out.println(s(r, u) - s(l - 1, u) - s(r, d - 1) + s(l - 1, d - 1));
	}

	private static long s(int x, int y) {
		if (x < 0 && y < 0) return calc(-x, -y) - calc(-x, 0) - calc(0, -y) + calc(0, 0);
		else if (y >= 0) return x >= 0 ? calc(x, y) : calc(-x, y) - calc(0, y);
		else return calc(x, -y) - calc(x, 0);
	}

	private static long calc(int ax, int ay) {
		int mn = min(ax, ay);
		int mx = max(ax, ay);
		int n = mn / 2;
		long k2 = 2L * n * (n + 1) + n + 1;
		long k1 = (mx / 2) - (mn / 2) * (mn + 1L);
		return k1 + k2;
	}
