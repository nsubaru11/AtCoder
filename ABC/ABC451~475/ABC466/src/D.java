...
public final class D {
	...
	private static void solve() {
		...
		boolean[] memoR = new boolean[n];
		boolean[] memoC = new boolean[n];
		int[][] rc = sc.nextIntMat(m, 2);
		int ans = 0;
		while (m-- > 0) {
			int ri = rc[m][0] - 1, ci = rc[m][1] - 1;
			if (!memoR[ri] && !memoC[ci]) ans++;
			memoR[ri] = memoC[ci] = true;
		}
		out.println(ans);
	}
