...
public final class E {
	...
	private static void solve() {
		...
		int b = sc.nextInt();
		int cnt = 0;
		Boolean[][] memo = new Boolean[m][m];
		fill(memo[0], false);
		for (int i = 1; i < m; i++) memo[i][0] = false;
		for (int x = 1; x < m; x++) {
			for (int y = 1; y < m; y++) {
				int sn2 = x, sn1 = y;
				Stack<Integer> stack = new Stack<>();
				while (memo[sn1][sn2] == null) {
					stack.push(sn1 * m + sn2);
					int sn3 = (a * sn1 + b * sn2) % m;
					memo[sn1][sn2] = sn3 != 0;
					if (sn3 == 0) break;
					sn2 = sn1;
					sn1 = sn3;
				...
				if (memo[sn1][sn2]) cnt++;
					...
					for (int v : stack) memo[v / m][v % m] = false;
				}
			}
		}
		out.println(cnt);
