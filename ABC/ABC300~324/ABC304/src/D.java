...
public final class D {
	...
	private static void solve() {
		...
		int min = 0, max = 0;
		if ((an + 1L) * (bn + 1) > n) {
			HashMap<Long, Integer> memo = new HashMap<>(n);
			for (int i = 0; i < n; i++) {
				int p = pq[0][i], q = pq[1][i];
				int ai = ~binarySearch(a, p);
				int bi = ~binarySearch(b, q);
				long abi = ai * (bn + 1L) + bi;
				int cnt = memo.merge(abi, 1, Integer::sum);
				if (max < cnt) max = cnt;
			}
		} else {
			int[] memo = new int[(an + 1) * (bn + 1)];
			for (int i = 0; i < n; i++) {
				int p = pq[0][i], q = pq[1][i];
				int ai = ~binarySearch(a, p);
				int bi = ~binarySearch(b, q);
