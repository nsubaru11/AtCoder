...
public final class C {
	...
	private static void solve() {
		...
		graph.addAll(m, sc::nextInt0, sc::nextInt0);
		int[][] memo = new int[n][1 << n];
		for (int i = 0; i < n; i++) {
			fill(memo[i], -1);
			memo[i][(1 << n) - 1] = 1;
		}
		int ans = dfs(graph, 0, 1, memo);
		out.println(ans);
	}

	private static int dfs(UndirectedGraph graph, int u, int flag, int[][] memo) {
		if (memo[u][flag] != -1) return memo[u][flag];
		memo[u][flag] = 0;
		for (int v : graph.adj(u)) {
			if ((flag >> v & 1) != 0) continue;
			memo[u][flag] += dfs(graph, v, flag | 1 << v, memo);
		}
		return memo[u][flag];
	}
