...
public final class C {
	...
	private static boolean dfs(int[] n, boolean[] d, boolean flag, int i) {
		if (i == n.length) return true;
		if (flag) {
			for (int j = 0; j < 10; j++) {
				if (d[j]) continue;
				n[i] = j;
				return dfs(n, d, true, i + 1);
			}
		} else {
			if (!d[n[i]]) return dfs(n, d, false, i + 1);
