...
public final class E {
	...
	private static void solve() {
		int n = sc.nextInt();
		int k = sc.nextInt();
		long[] dp = sc.nextLong(n);
		ArrayDeque<Integer> max_dq = new ArrayDeque<>(k + 1);
		for (int i = 0; i < n; i++) {
			if (!max_dq.isEmpty() && max_dq.peekFirst() < i - k) max_dq.pollFirst();
			if (!max_dq.isEmpty()) dp[i] = dp[i] + dp[max_dq.peekFirst()];
			while (!max_dq.isEmpty() && dp[max_dq.peekLast()] < dp[i]) max_dq.pollLast();
			max_dq.addLast(i);
		}
		out.println(dp[n - 1]);
	}
