...
public class E {
	...
	private static final FastPrinter out = new FastPrinter(false);

	public static void main(String[] args) {
		int n = sc.nextInt();
		int m = sc.nextInt();
		UnionFind uf = new UnionFind(n);
		HashMap<Integer, List<Pair>> hs = new HashMap<>();
		for (int i = 0; i < m; i++) {
			int a = sc.nextInt() - 1;
			int b = sc.nextInt() - 1;
			if (uf.isConnected(a, b)) {
				hs.computeIfAbsent(uf.find(a), k -> new ArrayList<>()).add(new Pair(a, b, i));
			} else {
				int findA = uf.find(a);
				int findB = uf.find(b);
				uf.union(a, b);
				if (findA == uf.find(a)) {
					if (hs.containsKey(findB)) {
						hs.computeIfAbsent(findA, k -> new ArrayList<>()).addAll(hs.get(findB));
						hs.remove(findB);
					}
				} else {
					if (hs.containsKey(findA)) {
						hs.computeIfAbsent(findB, k -> new ArrayList<>()).addAll(hs.get(findA));
						hs.remove(findA);
					}
				}
			}
		}
		out.println(uf.groupCount() - 1);
		int root = 0;
		List<Pair> pairs0 = null;
		for (int h: hs.keySet()) {
			if (root == 0) {
				root = ~h;
				pairs0 = hs.get(h);
			} else {
				Pair pair = pairs0.remove(0);
				uf.union(pair.a, h);
				out.print(pair.c + 1, pair.a + 1, h + 1).println();
				pairs0.addAll(hs.get(h));
			}
		}
		if (uf.groupCount() > 1) {
			root = ~root;
			for (int i = 0; i < n; i++) {
				if (!uf.isConnected(uf.find(i), root)) {
					Pair pair = pairs0.remove(0);
					uf.union(pair.a, i);
					out.print(pair.c + 1, pair.a + 1, i + 1).println();
				}
			}
		}
		out.flush();
	...
	private static final class FastPrinter {
		...
		public FastPrinter(int capacity, boolean autoFlush) {
			...
			sb = autoFlush ? null : new StringBuilder(capacity);
		}
