...
public class D {
	...
	private static void solve(final FastScanner sc, final FastPrinter out) {
		int h = sc.nextInt();
		int w = sc.nextInt();
		char[] s = sc.nextChars(h * w);
		int a = sc.nextInt() - 1;
		int b = sc.nextInt() - 1;
		int c = sc.nextInt() - 1;
		int d = sc.nextInt() - 1;
		out.println(new BFS_01(s, h, w).getShortestPathWeight(a, b, c, d));
	}
	...
	private static final class BFS_01 {
		private static final long INF = MAX_VALUE;
		private final int h, w;
		private final int hw;
		private final char[] s;

		public BFS_01(char[] s, int h, int w) {
			this.s = s;
			this.h = h;
			this.w = w;
			this.hw = h * w;
		...
		public long getShortestPathWeight(int a, int b, int c, int d) {
			int start = a * w + b;
			int end = c * w + d;

			int[] dv = {-1, -w, 1, w};
			long[] dist = new long[hw];
			fill(dist, INF);
			dist[start] = 0;

			ArrayDeque<Vertex> dq = new ArrayDeque<>();
			dq.addLast(new Vertex(start, 0));

			while (!dq.isEmpty()) {
				Vertex vertex = dq.pollFirst();
				int v = vertex.v;
				long cost = vertex.cost;
				...
				for (int k = 0; k < 4; k++) {
					...
					if (s[nv] == '.') {
						if (dist[nv] > cost) {
							dist[nv] = cost;
							dq.addFirst(new Vertex(nv, cost));
						}
						continue;
					}
					...
					if (dist[nv] >= cost + 1) {
						dist[nv] = cost + 1;
						dq.addLast(new Vertex(nv, cost + 1));
						// 2マス目もチェック
						if (isInRange(nv, dv[k])) {
							int nv2 = nv + dv[k];
							if (dist[nv2] > cost + 1) {
								dist[nv2] = cost + 1;
								dq.addLast(new Vertex(nv2, cost + 1));
							}
						}
					}
				}
		...
		private boolean isInRange(int v, int dv) {
			if (abs(dv) == 1) {
				int k = v % w + dv;
				return 0 <= k && k < w;
			}
			return 0 <= v + dv && v + dv < hw;
		}

		private record Vertex(int v, long cost) implements Comparable<Vertex> {
			@Override
