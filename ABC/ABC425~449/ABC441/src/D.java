...
public final class D {
	...
	private static void solve() {
		...
		dfs(l, 0, 0, s, t, graph, bs);
		int[] ans = new int[bs.cardinality()];
		for (int i = 0, j = 0; i < n; i++) {
			if (bs.get(i)) ans[j++] = i + 1;
		}
		out.print(ans).println();
	}

	private static void dfs(int l, int u, int c, int s, int t, DirectedGraph graph, BitSet bs) {
		if (l == 0) {
			if (s <= c && c <= t) bs.set(u);
			return;
		}
		for (int[] vc : graph.adj(u)) {
			int c2 = c + vc[1];
			int v = vc[0];
			if (c2 <= t) dfs(l - 1, v, c2, s, t, graph, bs);
		}
	}
	...
	private static final class DirectedGraph {
		// -------------- フィールド --------------
		private final int[] dest, next, first, inDegree, outDegree, cost;
		private final int n;
		...
		public void add(final int i, final int j, final int c) {
			dest[edgeCount] = j;
			next[edgeCount] = first[i];
			cost[edgeCount] = c;
			first[i] = edgeCount++;
		...
		public Iterable<int[]> adj(final int u) {
			return () -> new Iterator<>() {
